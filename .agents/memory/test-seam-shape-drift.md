...
**How to apply:** When a seam-faked route test fails with a "clean" wrong value (no thrown error, no console.error), diff the fake's return value against the real function's current return type FIRST — before instrumenting the route or suspecting DB/dedup logic. The same rule applies to route-intercepted browser fixtures: preserve list-item wrappers instead of flattening their nested values, or UI eligibility code can honestly render an empty state. Typing the fake as `typeof realFn` in the test would catch this at compile time; prefer that when writing new seams.
