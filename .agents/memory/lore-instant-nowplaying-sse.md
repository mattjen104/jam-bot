...
- WebPlayer surface: `webplayer/nowPlayingStream.ts` owns ONE refcounted EventSource shared by every `useWpOnAir` consumer; pushed spins merge idempotently into the `["wp","onair"]` react-query cache (setQueryData), and stream health only stretches/restores the poll interval — polling itself is never removed. spin-changed events carry `observedAt`+`confidence` so no follow-up fetch is needed.
- Boot-only migrations (run in src/index.ts) are NOT automatically in the DB test suite — any column persistSpin/readers depend on must also be added to `test/globalSetup.ts` or new DB tests fail with "column does not exist" while prod works fine.
