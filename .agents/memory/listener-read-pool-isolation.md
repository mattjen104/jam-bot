...
**How to apply:** Reserve the pool for the minimal live read queries only. Bound optional identity and personalization separately; for on-air crossing copy, match small taste sets against already-bounded current/recent spin evidence instead of rescanning full history. Keep archive semantics distinct from home-only fast paths, and test a real signed listener with a cold personalization cache while the general pool is saturated.