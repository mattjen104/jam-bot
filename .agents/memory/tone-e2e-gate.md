...
**Merge-splice hazard:** task merges repeatedly corrupt the tone spec (duplicate test titles abort Playwright discovery; assertions swap between tests). Canonical shape = active control + active positive click + active strict boundary + one quarantined fixme; restore from the last good revision rather than editing splice damage in place, and make the attempt helper settle deterministically (clock-based ended fallback) instead of only widening timeouts.
