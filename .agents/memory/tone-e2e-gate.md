...
**Pre-existing boundary-test failure (Aug 2026):** the spec "pre-unlocked tone element still plays >5s later" (interstitialTone.spec.ts:249) fails on main independent of code changes — Chromium in this environment no longer honors the muted-play pre-unlock past the transient-activation window. Verify against the parent commit before blaming your change; if it fails there too, complete with a documented skip_validation_reason.
