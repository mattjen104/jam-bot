...
export const stationsTable = pgTable("stations", {
   ...
   * A non-null value with a null/blank logoUrl records a confirmed absence.
   */
  ...
  /**
   * Last completed station-icon discovery attempt. Null = never checked.
   * A non-null value with a null/blank stationIconUrl records a confirmed absence.
   */
...
// ---------------------------------------------------------------------------
// Canonical credits layer
// ---------------------------------------------------------------------------

/**
 * MusicBrainz works are deliberately separate from recordings.  A recording
 * can point at more than one work (translations, medleys, arrangements), and
 * collapsing that relationship into a single writer field loses source facts.
 */
export const musicbrainzWorksTable = pgTable("musicbrainz_works", {
  mbid: text("mbid").primaryKey(),
  title: text("title"),
  language: text("language"),
  source: text("source").notNull().default("musicbrainz"),
  parserVersion: text("parser_version").notNull().default("credits-v1"),
...
export type MusicbrainzWork = typeof musicbrainzWorksTable.$inferSelect;
export type InsertMusicbrainzWork = typeof musicbrainzWorksTable.$inferInsert;

export const recordingWorksTable = pgTable(
  "recording_works",
  {
    recordingMbid: text("recording_mbid")
      .notNull()
    ...
    workMbid: text("work_mbid")
      .notNull()
      .references(() => musicbrainzWorksTable.mbid, { onDelete: "cascade" }),
  ...
  (t) => [
    primaryKey({ name: "recording_works_pk", columns: [t.recordingMbid, t.workMbid] }),
    index("recording_works_work_idx").on(t.workMbid),
  ],
);

export type RecordingWork = typeof recordingWorksTable.$inferSelect;
export type InsertRecordingWork = typeof recordingWorksTable.$inferInsert;

/** Canonical release/edition facts. Labels belong to this table's releases,
 * never to a recording, so reissues and compilations remain distinct. */
export const musicbrainzReleasesTable = pgTable(
  ...
  (t) => [
    index("musicbrainz_releases_group_idx").on(t.releaseGroupMbid),
  ],
);

export type MusicbrainzRelease = typeof musicbrainzReleasesTable.$inferSelect;
export type InsertMusicbrainzRelease = typeof musicbrainzReleasesTable.$inferInsert;

export const musicbrainzLabelsTable = pgTable("musicbrainz_labels", {
  mbid: text("mbid").primaryKey(),
  name: text("name"),
  source: text("source").notNull().default("musicbrainz"),
  fetchedAt: timestamp("fetched_at"),
...
export type MusicbrainzLabel = typeof musicbrainzLabelsTable.$inferSelect;
export type InsertMusicbrainzLabel = typeof musicbrainzLabelsTable.$inferInsert;

export const releaseLabelsTable = pgTable(
  "release_labels",
  {
    releaseMbid: text("release_mbid")
      .notNull()
      .references(() => musicbrainzReleasesTable.mbid, { onDelete: "cascade" }),
    labelMbid: text("label_mbid")
      .notNull()
      .references(() => musicbrainzLabelsTable.mbid, { onDelete: "cascade" }),
  ...
  (t) => [
    primaryKey({ name: "release_labels_pk", columns: [t.releaseMbid, t.labelMbid] }),
    index("release_labels_label_idx").on(t.labelMbid),
  ],
);

export type ReleaseLabel = typeof releaseLabelsTable.$inferSelect;
export type InsertReleaseLabel = typeof releaseLabelsTable.$inferInsert;

/**
 * Normalized person/role facts. `credit_key` is generated by the parser and
 * makes retries idempotent even when the same person appears through several
 * MusicBrainz relationship arrays.
 */
export const recordingCreditsTable = pgTable(
  "recording_credits",
  {
    creditKey: text("credit_key").primaryKey(),
    recordingMbid: text("recording_mbid")
      .notNull()
      .references(() => recordingsTable.mbid, { onDelete: "cascade" }),
    workMbid: text("work_mbid").references(() => musicbrainzWorksTable.mbid, {
      onDelete: "set null",
    }),
    artistMbid: text("artist_mbid"),
    ...
    parserVersion: text("parser_version").notNull().default("credits-v1"),
    completeness: text("completeness").notNull().default("partial"),
    attemptStatus: text("attempt_status").notNull().default("success"),
    fetchedAt: timestamp("fetched_at"),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
  },
  (t) => [
...
export type RecordingCredit = typeof recordingCreditsTable.$inferSelect;
export type InsertRecordingCredit = typeof recordingCreditsTable.$inferInsert;

/**
 * Kept-only enrichment queue. A row is unique per recording, allowing an
 * explicit keep to safely enqueue the same recording repeatedly.
 */
export const creditEnrichmentQueueTable = pgTable(
  "credit_enrichment_queue",
  {
    recordingMbid: text("recording_mbid")
      .primaryKey()
      .references(() => recordingsTable.mbid, { onDelete: "cascade" }),
    scope: text("scope").notNull().default("recording"),
    status: text("status").notNull().default("pending"),
  ...
  (t) => [
    index("credit_enrichment_queue_ready_idx").on(
      t.status,
      t.nextAttemptAt,
      t.priority,
    ),
...
export type CreditEnrichmentQueue =
  typeof creditEnrichmentQueueTable.$inferSelect;
export type InsertCreditEnrichmentQueue =
