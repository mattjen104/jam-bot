 ...
 * The DDL and DML steps are interdependent (NOT NULL constraint in step 3
 * depends on the backfill in steps 2a/2b), so the entire function is gated
 * as a unit. On first run a row is inserted into `migration_completions`
 * inside the same transaction as all the work; subsequent boots find that row
...
export async function applyDeviceIdentityMigration(): Promise<void> {
  ...
  const completionCheck = await db.execute(
    sql`SELECT 1 FROM migration_completions WHERE name = 'applyDeviceIdentityMigration' LIMIT 1`,
  );
  if ((completionCheck.rows?.length ?? 0) > 0) {
    console.info("[migration] device identity migration: already complete, skipping");
    return;
  }
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.execute(sql`
      ...
      VALUES ('applyDeviceIdentityMigration')
      ON CONFLICT (name) DO NOTHING
    `);
  });

  console.info("[migration] device identity migration: OK");
}
