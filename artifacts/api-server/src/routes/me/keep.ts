...
/** Client context that is useful for analytics but cannot attribute a play. */
const KEEP_CONTEXT_KEYS = ["surface", "entryPoint"] as const;

type KeepSpinProvenance = {
  stationSlug: string;
  stationName: string;
 ...
 * row and its station/show/picker joins. Client input is limited to the two
 * non-attribution context fields used for analytics.
 *
 * Exported for the legacy provenance backfill so new writes and repairs
 * cannot drift into separate policies.
 */
export function buildKeepProvenance(
  override: unknown,
  spin: KeepSpinProvenance | null,
): LibraryItemProvenance {
  const provenance: LibraryItemProvenance = { kind: "keep" };

  if (override && typeof override === "object" && !Array.isArray(override)) {
    const input = override as Record<string, unknown>;
    for (const key of KEEP_CONTEXT_KEYS) {
      if (Object.prototype.hasOwnProperty.call(input, key)) {
        provenance[key] = input[key];
      }
    }
  }

  if (spin) {
    provenance.stationSlug = spin.stationSlug;
    provenance.stationName = spin.stationName;
    if (spin.showName != null) provenance.showName = spin.showName;
    if (spin.djName != null) provenance.djName = spin.djName;
    if (spin.pickerHandle != null) provenance.pickerHandle = spin.pickerHandle;
    if (spin.pickerName != null) provenance.pickerName = spin.pickerName;
    provenance.playedAt = spin.playedAt.toISOString();
  }
...
router.post(
  ...
  h(async (req, res) => {
    ...
    if (!mbid && spinId != null) {
      ...
      if (spin.mbid) {
        const provenance = buildKeepProvenance(provenanceOverride, spin);
        await db
      ...
      const [spinPendingCount] = await db
        ...
        .where(
          and(
            ...
            isNull(pendingKeepsTable.promotedAt),
          ),
        );
      const showRecoveryHint =
        (spinLibCount?.n ?? 0) + (spinPendingCount?.n ?? 0) === 3;

      return res.json({
        keptToLore: promotedAt != null,
        pendingKept: true,
        mirrors: [],
        showRecoveryHint,
      });
    ...
    let keepSpin: KeepSpinProvenance | null = null;
    if (spinId != null) {
      const [s] = await db
        .select({
          id: spinsTable.id,
      ...
      if (s) {
        keepSpin = {
          ...
          showName: s.showName,
          djName: s.djName,
          pickerHandle: s.pickerHandle,
          pickerName: s.pickerName,
          playedAt: s.playedAt,
        };
      }
    }
    const provenance = buildKeepProvenance(provenanceOverride, keepSpin);
    const keepSpinId = keepSpin ? spinId! : null;
    ...
    const enabledTargets = await db
      ...
      .where(
        and(
          eq(keepTargetsTable.userId, user.id),
          eq(keepTargetsTable.enabled, true),
        ),
      );
...
router.delete(
  ...
  h(async (req, res) => {
    const user = (req as AuthedRequest).loreUser;
    const spinId = parseInt(
      typeof req.params.spinId === "string" ? req.params.spinId : "",
      10,
    );
    ...
    await db
      ...
      .where(
        and(
          eq(pendingKeepsTable.userId, user.id),
          eq(pendingKeepsTable.spinId, spinId),
        ),
      );
      ...
      await db
        ...
        .where(
          and(
            ...
            eq(libraryItemsTable.mbid, spin.mbid),
          ),
        );
    }
 ...
 */
router.get(
  "/me/keep/pending-status",
  h(async (req, res) => {
    const user = (req as AuthedRequest).loreUser;
    const rawIds =
      typeof req.query.spinIds === "string" ? req.query.spinIds : "";
    ...
    const rows = await db
      .select({
        spinId: pendingKeepsTable.spinId,
        promotedAt: pendingKeepsTable.promotedAt,
      })
      .from(pendingKeepsTable)
      .where(
        and(
    ...
    const savedSpinIds = rows
      .filter((r) => r.promotedAt != null)
      .map((r) => r.spinId);
    const pendingSpinIds = rows
      .filter((r) => r.promotedAt == null)
      .map((r) => r.spinId);
...
router.get(
  ...
  h(async (req, res) => {
    const user = (req as AuthedRequest).loreUser;
    const raw =
      typeof req.query["mbids"] === "string" ? req.query["mbids"] : "";
