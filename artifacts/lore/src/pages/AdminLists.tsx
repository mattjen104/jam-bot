    ...
    <section className="mt-8 rounded-2xl border border-card-border bg-card p-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          ...
          disabled={loading}
          className="shrink-0 inline-flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-base font-normal text-primary-foreground shadow-sm transition-opacity disabled:opacity-40"
        >
          {loading ? (
      ...
      {error && (
        <div className="mt-4 flex items-center gap-2 rounded-lg bg-destructive/10 px-3 py-2 text-base text-destructive-foreground">
      ...
      {result && (
        <div className="mt-4 rounded-lg bg-secondary/40 px-4 py-3">
          <div className="flex flex-wrap gap-6 font-mono text-base">
            <span>
              <span className="text-zinc-400">{result.enrolled}</span>
              <span className="ml-1.5 text-muted-foreground">enrolled with feed</span>
            </span>
            <span>
              <span className="text-zinc-400">{result.linkOutOnly}</span>
  ...
  return (
    <section className="mt-4 rounded-2xl border border-card-border bg-card p-6">
      ...
      <div className="mt-4 flex items-center gap-3">
        <label className="font-mono text-[13px] text-muted-foreground">
          Limit
        </label>
        <input
          type="number"
          ...
          className="w-20 rounded-lg border border-border bg-secondary/40 px-3 py-1.5 text-base text-foreground focus:border-primary focus:outline-none"
        />
        <button
          type="button"
          onClick={() => void handleBatch()}
          disabled={loading}
          className="inline-flex items-center gap-2 rounded-full bg-secondary px-4 py-1.5 text-base font-normal text-foreground transition-colors hover:bg-secondary/80 disabled:opacity-40"
        >
      ...
      {error && (
        <div className="mt-3 flex items-center gap-2 rounded-lg bg-destructive/10 px-3 py-2 text-base text-destructive-foreground">
          <AlertCircle className="h-4 w-4 shrink-0" />
          {error}
        </div>
      )}
            ...
            <ul className="mt-2 flex flex-col gap-1">
              {result.outcomes.map((o) => (
                <li key={o.id} className="flex items-start gap-2 rounded-lg bg-secondary/30 px-3 py-2">
                  <span className={`mt-0.5 shrink-0 rounded-full px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wide ${
                    o.status === "extracted"
                  ...
                  <span className="min-w-0 flex-1 text-sm text-foreground leading-snug">{o.title}</span>
                  {o.note && (
                    <span className="shrink-0 text-[13px] text-muted-foreground/60">{o.note}</span>
                  )}
                </li>
              ))}
            </ul>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1 text-base text-primary hover:bg-primary/20"
          >
      ...
      {!loading && sources && sources.length > 0 && (
        <div className="mt-4 overflow-x-auto">
          <table className="w-full text-base">
            <thead>
              <tr className="border-b border-border font-mono text-[12px] uppercase tracking-wide text-muted-foreground/70">
              ...
              {sources.map((s) => (
                <tr key={s.id}>
                  ...
                  <td className="py-2 pr-4">
                    <span className="rounded-full bg-secondary/50 px-2 py-0.5 font-mono text-[12px] text-muted-foreground">
                      {s.kind}
                    </span>
                  </td>
                  <td className="py-2 pr-4">
                  ...
                  <td className="py-2 text-right font-mono text-[13px] text-muted-foreground/60">
                    {s.pickerId ?? "—"}
                  </td>
                </tr>
              ))}
    ...
    >
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="flex flex-col gap-1">
          <label className="font-mono text-[12px] uppercase tracking-wide text-muted-foreground">
            Name *
          </label>
          <input
            value={name}
            ...
            className="rounded-lg border border-border bg-secondary/40 px-3 py-1.5 text-base text-foreground placeholder:text-muted-foreground/50 focus:border-primary focus:outline-none"
          />
        </div>
        <div className="flex flex-col gap-1">
          <label className="font-mono text-[12px] uppercase tracking-wide text-muted-foreground">
          ...
          <select
            ...
            className="rounded-lg border border-border bg-secondary/40 px-3 py-1.5 text-base text-foreground focus:border-primary focus:outline-none"
          >
      ...
      <div className="mt-4 flex items-center gap-2">
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-full bg-primary px-4 py-1.5 text-base font-normal text-primary-foreground disabled:opacity-40"
        >
        ...
        <button
          type="button"
          onClick={onCancel}
          className="inline-flex items-center gap-1 rounded-full border border-border px-3 py-1.5 text-base text-muted-foreground hover:text-foreground"
        >
      ...
      >
        ...
        <div>
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-2 text-base font-normal text-primary-foreground shadow-sm transition-opacity disabled:opacity-40"
          >
            {scraping ? (
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
            ) : (
        ...
        <div className="mt-4">
          <div className="rounded-lg bg-secondary/40 px-4 py-3">
            <div className="flex flex-wrap gap-6 font-mono text-base">
              ...
              <span>
                <span className="text-zinc-400">{scrapeResult.resolved}</span>
                <span className="ml-1.5 text-muted-foreground">exact</span>
              </span>
              <span>
                <span className="text-zinc-400">{scrapeResult.fuzzy}</span>
            ...
            {scrapeResult.error && (
              <p className="mt-1 text-sm text-destructive-foreground">{scrapeResult.error}</p>
            )}
          </div>
          ...
          {(scrapeResult.fuzzy > 0 || scrapeResult.unresolved > 0) && (
            <button
              ...
              className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1.5 text-base text-primary hover:bg-primary/20"
            >
              {showEntries ? (
  ...
  return (
    <div className="mt-3 rounded-xl border border-border bg-secondary/20 p-4">
      ...
      {!loading && entries && entries.length === 0 && (
        <p className="mt-3 text-base text-muted-foreground">No entries to review.</p>
      )}
            ...
            <tbody className="divide-y divide-border">
              {entries.map((e) => (
                <tr key={e.id} className={e.confirmed ? "opacity-50" : ""}>
                  <td className="py-2 pr-3 font-mono text-[13px] text-muted-foreground/60">
                    {e.rank ?? "—"}
                  </td>
                  ...
                  <td className="py-2 pr-3 text-center">
                    <span
                      className={`rounded-full px-2 py-0.5 font-mono text-[12px] uppercase tracking-wide ${
                  ...
                  <td className="py-2 text-right">
                    {!e.confirmed && e.confidence === "exact" && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-full bg-zinc-500/15 px-2 py-0.5 font-mono text-[12px] text-zinc-400 hover:bg-zinc-500/25"
                      >
                    ...
                    {e.confirmed && (
                      <span className="font-mono text-[12px] text-zinc-400/60">✓</span>
                    )}
                  </td>
                </tr>
              ))}
