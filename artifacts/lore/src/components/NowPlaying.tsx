      ...
      <div className="relative -mt-10 p-6">
        {np ? (
          <>
            ...
            {rec && <ListProvenance mbid={rec.mbid} />}
 ...
 * "via OK Computer — #3 on The Quietus 2025". Only exact/confirmed entries
 * are surfaced; returns null when none have been scraped yet.
 */
function ListProvenance({ mbid }: { mbid: string }) {
      ...
      <ul className="space-y-2">
        {items.map((item) => {
          const albumLine = [
            ...
            .filter(Boolean)
            .join(" ");
          const rankLine = item.isRanked && item.rank != null
            ? `#${item.rank}`
            : "listed";
          ...
          return (
            <li key={`${item.listId}-${item.releaseGroupMbid}`} className="text-sm leading-snug">
              {albumLine && (
                <span className="font-medium text-foreground">{albumLine} — </span>
              )}
              <a
                href={item.listUrl}
                target="_blank"
