...
import { ArrowUpRight, Loader2, Music2, Play } from "lucide-react";
...
/**
 * One selector card — same visual language as a station card.
 * The 2×2 artwork mosaic doubles as the play button.
 */
function SelectorCard({ item }: { item: PickerDialItem }) {
  ...
  return (
    <li>
      <div className="hover-elevate group flex items-center gap-4 rounded-xl border border-card-border bg-card p-3 pr-4 transition-colors">
        ...
        {/* Right rail: follow selector + archive link */}
        <div
...
/**
 * The selectors section of the dial: a heading, "see all" link, and one card
 * per selector. Renders nothing when the list is empty (e.g. on initial load).
 */
export function SelectorDial({ items }: { items: PickerDialItem[] }) {
  ...
  return (
    <section className="mt-6">
      <div className="mb-3 flex items-baseline justify-between">
        ...
        <Link
          href="/selectors"
          className="inline-flex items-center gap-1 font-mono text-[10px] uppercase tracking-wide text-muted-foreground hover:text-primary"
        >
          All selectors
          <ArrowUpRight className="h-3 w-3" />
        </Link>
      </div>
      <ul className="flex flex-col gap-2" data-testid="selector-dial">
        {items.map((item) => (
          <SelectorCard key={item.picker.handle} item={item} />
        ))}
      </ul>
    </section>
  );
